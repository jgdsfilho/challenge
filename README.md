...
## Technical Details
### Tenants
- **Get /tenants/{tenant_id}**: Retrieves a client by ID.

- **Get /tenants/?**: Retrieve client by email.
    - Query Params:
      - **email**: Retrieves a client by email.
        - Example: /tenants/?email=test@test.com
...
- **Get /products/?**: Retrieves a product by sku.
    - Query Params:
      - **sku**: Retrieves a product by sku.
        - Example: /products/?sku=123456
...
### ProductPrices
- **Get /products/{product_id}/prices**: Retrieves price for a product.
...
### Contracts
- **Get /contracts/tenants/{tenant_id}**: Retrieves all contracts for a client.
...
### Billings
- **Get /billings/tenants/{tenant_id}/**: Retrieves all billings for a client.
    - Query Params:
      - **billing_date**: Retrieves all billings for a client for a specific month.
        - Example: /billings/tenants/123e4567-e89b-12d3-a456-426614174000?billing_date=2024-01-01
- **Post /billings**: Creates a new billing record for a client. The billing includes a tenant and its billing details. The system does not allow a client to have more than one billing record for the same month.
   - Body:
     ```json
     {
       "tenant_id": "string",

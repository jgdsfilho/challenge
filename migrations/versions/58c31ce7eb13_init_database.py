...
Revision ID: 58c31ce7eb13
Revises: 
Create Date: 2025-01-13 16:08:25.243908
...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_products_product_sku"), "products", ["product_sku"], unique=True
    )
    op.create_table(
        "tenants",
        sa.Column("id", sa.Uuid(), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("free_allocation", sa.Numeric(), nullable=False),
        sa.Column("is_active", sa.Boolean(), nullable=False),
        sa.ForeignKeyConstraint(
            ["product_id"],
            ["products.id"],
